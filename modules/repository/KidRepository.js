...
module.exports = {
    ...
    updateKidInformation: function (kidId, kidData, callback) {
        databaseConfig.getSession().query('UPDATE kid k SET k.first_name = ?, k.last_name = ?, k.avatar_image = ? WHERE k.id = ?', [kidData.firstName, kidData.lastName, kidData.avatarImage, kidId], (err, result) => {
            if (err) {
                console.log(err);
                return callback(null);
            }
            return this.getKidInformation(kidId, callback);
        });
        databaseConfig.closeConnection();
    },
